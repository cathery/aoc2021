...
int main()
{
	...
	for (const auto& CrabPosition : CrabPositions)
	{
		...
		{
			const std::uintmax_t DistanceToPoint = MapIndex > CrabPosition ? MapIndex - CrabPosition : CrabPosition - MapIndex;

			//Find the triangular number
			PathfindingMap[MapIndex] += DistanceToPoint * (DistanceToPoint + 1) / 2;
		}
	}
