...
{
	std::shared_ptr<Pair> NewPair = std::make_shared<Pair>();
	First->Outer = NewPair.get();
	Second->Outer = NewPair.get();
	NewPair->Left = First;
...
int main()
{
	...
	std::shared_ptr<Element> CurrentResult;
	...
	while (Input)
	{
		...
		else if (PeekedChar == '\n')
		{
			if (!CurrentResult)
			{
				CurrentResult = std::move(Outermost);
			}
			else
			{
				CurrentResult = Element::Add(CurrentResult, Outermost);
			}
			Outermost.reset();
	...
	std::cout << CurrentResult->Magnitude() << std::endl;
}